...
.DESCRIPTION
    This script is designed to provide a user-friendly, menu-driven interface for triggering the various backup scan tools. 
    It allows the user to choose from a number of options, each corresponding to a specific type of backup scan. 
    
    Details can be found on GitHub https://github.com/yetanothermightytool/powershell/tree/master/vbr/backup-scanning-tools
.NOTES  
    File Name  : backup-scanning-tools-menu.ps1
    Author     : Stephan "Steve" Herzig
    Requires   : Powershell, all scripts and prerequisites for the individual scanning scripts
.VERSION
    1.0
#>
# General variables
...
do {
    ...
    switch ($choice) {
        "1" {
            ...
            Write-Host "╚════════════════════════════════════════╝" -ForegroundColor White
            Write-Host ""
            Write-Host "This option mounts the selected restore point of a Veeam VM or Agent backup using the Data Integration API function to a Linux server (mount server) and runs an anti-virus file-level scan using ClamAV." -ForegroundColor Cyan
            Write-Host ""
        ...
        "2" {
            ...
            Write-Host "╚════════════════════════════════════════╝" -ForegroundColor White
            Write-Host ""
            Write-Host "This option mounts the selected restore point of a Veeam VM or Agent backup using the Data Integration API function to a Linux server (mount server) and runs a YARA scan." -ForegroundColor Cyan
            Write-Host ""
            $param1 = Read-Host "Host to attach backup to "
            $param2 = Read-Host "Host to scan             "
            $param3 = Read-Host "Backup Job Name          "
        ...
        "3" {
            ...
            Write-Host "╚══════════════════════════════════════════════════════╝" -ForegroundColor White
            Write-Host ""
            Write-Host "This option is specific to Instant VM Disk Recovery and allows the user to attach disk(s) to a virtual machine (VM) for scanning." -ForegroundColor Cyan
            Write-Host ""
            ...
            $param2 = Read-Host "Host to scan            "
            $param3 = Read-Host "Backup Job Name         "
            $param4 = Read-Host "vCenter Hostname        "
            $scriptPath = "$scanningToolsPath\vbr-instantdiskrecovery.ps1"
            Write-Host ""
            Write-Host "Start script"  -ForegroundColor White
            Sleep 3
            & $scriptPath -Mounthost $param1 -Scanhost $param2 -Jobname $param3 -vCenter $param4
            Write-Host ""
            Pause
            [System.Console]::Clear()
        }
        "4" {
            ...
            Write-Host "╚════════════════════════════════════════╝" -ForegroundColor White
            Write-Host ""
            Write-Host "This option initiates a MS Defender scan on a NAS backup using a designated backup job name." -ForegroundColor Cyan
            Write-Host ""
